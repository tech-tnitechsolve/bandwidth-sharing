...
files_to_be_removed=($adnade_containers_file $adnade_file $chrome_containers_file $connection_state_file $container_names_file $custom_chrome_file $custom_firefox_file $dns_resolver_file $dnscrypt_config_file $earn_fm_config_file $ebesucher_file $firefox_containers_file $hickory_dns_config_file $mysterium_file $networks_file $process_id_file $subnets_file $uprock_file $ur_data_proxies_file $ur_proxies_file)
...
curl -4 -sSL -A "Mozilla/5.0" -o "$dnscrypt_tar_file" "$DNSCRYPT_URL"
...
# Download Hickory DNS file (Vá lỗi mirror & auto fallback)
download_hickory_dns() {
  case "$CPU_ARCH" in
    x86_64 | amd64) HICKORY_ARCH="x86_64" ;;
    i686 | i386)     HICKORY_ARCH="x86" ;;
    armv7l | armhf)  HICKORY_ARCH="armv7" ;;
    arm64 | aarch64) HICKORY_ARCH="aarch64" ;;
...
download_iptables() {
  case "$CPU_ARCH" in
    x86_64 | amd64) IPTABLES_ARCH="x86_64" ;;
    i686 | i386)     IPTABLES_ARCH="x86" ;;
    armv7l | armhf)  IPTABLES_ARCH="armv7" ;;
    arm64 | aarch64) IPTABLES_ARCH="aarch64" ;;
    *) echo -e "${RED}Unsupported architecture: $CPU_ARCH${NOCOLOUR}"; exit 1 ;;
  esac
  ...
  if [[ $i && $proxy && "$START_ONLY" != true ]]; then
    ...
    if [ "$vpn_enabled" = true ];then
      ...
      if [[ "$USE_SOCKS5_DNS" == "true" && "$proxy" == socks5://* ]]; then
        ...
        local SOCKS_AUTH_PARAMS=""
        if [[ -n "$SOCKS_USER" && -n "$SOCKS_PASS" ]]; then
          SOCKS_AUTH_PARAMS="-e SOCKS5_USERNAME=$SOCKS_USER -e SOCKS5_PASSWORD=$SOCKS_PASS"
        fi
        if [[ "$ENABLE_LOGS" != true ]]; then
          TUN_LOG_PARAM="warn"
        fi
        docker_parameters=($HOST_NAME $LOGS_PARAM $TUN_DNS_VOLUME $MAX_MEMORY_PARAM $MEMORY_RESERVATION_PARAM $MEMORY_SWAP_PARAM $CPU_PARAM $CUSTOM_NETWORK -e LOG_LEVEL=$TUN_LOG_PARAM --mount type=bind,source=/dev/net/tun,target=/dev/net/tun --cap-add=NET_ADMIN $combined_ports -e SOCKS5_ADDR="$SOCKS_ADDR" -e SOCKS5_PORT="$SOCKS_PORT" $SOCKS_AUTH_PARAMS -e ICMP='reply' --sysctl net.ipv6.conf.all.disable_ipv6=1 --sysctl net.ipv6.conf.default.disable_ipv6=1 --no-healthcheck ghcr.io/heiher/hev-socks5-tunnel:$HEVSOCKS_VERSION)
        execute_docker_command "Proxy" "tun$UNIQUE_ID$i" "${docker_parameters[@]}"
      elif [[ "$USE_SOCKS5_DNS" != "true" && "$USE_DNS_OVER_HTTPS" != "true" && "$proxy" =~ ^(http|https|socks4|socks5):// ]]; then
        if [[ "$ENABLE_LOGS" != true ]]; then
          TUN_LOG_PARAM="off"
        ...
        if [ "$USE_DNSCRYPT" != false ]; then
          doh_volume="--mount type=bind,source=$PWD/dnscrypt-proxy,target=/proxy-dns/dnscrypt-proxy --mount type=bind,source=$PWD/dns-config.toml,target=/proxy-dns/dns-config.toml"
          docker_parameters=($HOST_NAME $LOGS_PARAM $TUN_DNS_VOLUME $MAX_MEMORY_PARAM $MEMORY_RESERVATION_PARAM $MEMORY_SWAP_PARAM $CPU_PARAM $CUSTOM_NETWORK -e LOG_LEVEL=$TUN_LOG_PARAM --mount type=bind,source=/dev/net/tun,target=/dev/net/tun --mount type=bind,source=$PWD/iptables.apk,target=/iptables/iptables.apk --mount type=bind,source=$PWD/libmnl.apk,target=/iptables/libmnl.apk --mount type=bind,source=$PWD/libnftnl.apk,target=/iptables/libnftnl.apk $doh_volume --cap-add=NET_ADMIN $combined_ports -e SOCKS5_ADDR="$SOCKS_ADDR" -e SOCKS5_PORT="$SOCKS_PORT" $SOCKS_AUTH_PARAMS -e ICMP='reply' --sysctl net.ipv6.conf.all.disable_ipv6=1 --sysctl net.ipv6.conf.default.disable_ipv6=1 --no-healthcheck --entrypoint sh ghcr.io/heiher/hev-socks5-tunnel:$HEVSOCKS_VERSION -c "apk add --allow-untrusted --no-network /iptables/libmnl.apk && apk add --allow-untrusted --no-network /iptables/libnftnl.apk && apk add --allow-untrusted --no-network /iptables/iptables.apk && iptables -t nat -A PREROUTING -p udp --dport 53 -j DNAT --to-destination 127.0.0.1:53 && iptables -t nat -A OUTPUT -p udp --dport 53 -j DNAT --to-destination 127.0.0.1:53 && chmod +x /proxy-dns/dnscrypt-proxy && (/proxy-dns/dnscrypt-proxy -config /proxy-dns/dns-config.toml &) && /entrypoint.sh")
        else
          doh_volume="--mount type=bind,source=$PWD/hickory-dns-config.toml,target=/proxy-dns/hickory-dns-config.toml"
          docker_parameters=($HOST_NAME $LOGS_PARAM $TUN_DNS_VOLUME $MAX_MEMORY_PARAM $MEMORY_RESERVATION_PARAM $MEMORY_SWAP_PARAM $CPU_PARAM $CUSTOM_NETWORK -e LOG_LEVEL=$TUN_LOG_PARAM --mount type=bind,source=/dev/net/tun,target=/dev/net/tun --mount type=bind,source=$PWD/iptables.apk,target=/iptables/iptables.apk --mount type=bind,source=$PWD/libmnl.apk,target=/iptables/libmnl.apk --mount type=bind,source=$PWD/libnftnl.apk,target=/iptables/libnftnl.apk --mount type=bind,source=$PWD/libgcc.apk,target=/iptables/libgcc.apk --mount type=bind,source=$PWD/hickory-dns.apk,target=/iptables/hickory-dns.apk $doh_volume --cap-add=NET_ADMIN $combined_ports -e SOCKS5_ADDR="$SOCKS_ADDR" -e SOCKS5_PORT="$SOCKS_PORT" $SOCKS_AUTH_PARAMS -e ICMP='reply' --sysctl net.ipv6.conf.all.disable_ipv6=1 --sysctl net.ipv6.conf.default.disable_ipv6=1 --no-healthcheck --entrypoint sh ghcr.io/heiher/hev-socks5-tunnel:$HEVSOCKS_VERSION -c "apk add --allow-untrusted --no-network /iptables/libmnl.apk && apk add --allow-untrusted --no-network /iptables/libnftnl.apk && apk add --allow-untrusted --no-network /iptables/iptables.apk && apk add --allow-untrusted --no-network /iptables/libgcc.apk && apk add --allow-untrusted --no-network /iptables/hickory-dns.apk && iptables -t nat -A PREROUTING -p udp --dport 53 -j DNAT --to-destination 127.0.0.1:53 && iptables -t nat -A OUTPUT -p udp --dport 53 -j DNAT --to-destination 127.0.0.1:53 && (RUST_LOG=off hickory-dns --config /proxy-dns/hickory-dns-config.toml &) && /entrypoint.sh")
        fi
        execute_docker_command "Proxy" "tun$UNIQUE_ID$i" "${docker_parameters[@]}"
      else
...
# Starting Ebesucher Chrome container
  if [[ $EBESUCHER_USERNAME && "$EBESUCHER_USE_CHROME" = true ]]; then
    ...
    if [ "$container_pulled" = false ]; then
      ...
      docker_parameters=($LOGS_PARAM $DNS_VOLUME $MAX_MEMORY_PARAM $MEMORY_RESERVATION_PARAM $MEMORY_SWAP_PARAM $CPU_PARAM --mount type=bind,source=/var/run/docker.sock,target=/var/run/docker.sock --mount type=bind,source=$PWD,target=/chrome --network none docker:cli /bin/sh -c 'cd /chrome && chmod +x /chrome/restart.sh && while true; do sleep 3600; /chrome/restart.sh --restartChrome; done')
...
# Starting PacketShare container
  if [[ $PACKETSHARE_EMAIL && $PACKETSHARE_PASSWORD ]]; then
    local ps_platform=""
    if [ "$CPU_ARCH" == "aarch64" ] || [ "$CPU_ARCH" == "arm64" ]; then
      ps_platform="--platform=linux/amd64"
    fi
    if [ "$container_pulled" = false ]; then
  ...
  if [[ $EARN_FM_API && "$USE_EARN_FM_FLEETSHARE" = true ]]; then
	...
	EOF
        ...
        docker_parameters=($LOGS_PARAM $DNS_VOLUME $MAX_MEMORY_PARAM $MEMORY_RESERVATION_PARAM $MEMORY_SWAP_PARAM $CPU_PARAM --mount type=bind,source=/var/run/docker.sock,target=/var/run/docker.sock --mount type=bind,source=$PWD,target=/earnfm --network none docker:cli /bin/sh -c 'cd /earnfm && chmod +x /earnfm/restart.sh && while true; do sleep 86400; /earnfm/restart.sh --restartEarnFM; done')
  ...
  if [[ $NP_COOKIE ]]; then
    local np_platform=""
    if [ "$CPU_ARCH" == "aarch64" ] || [ "$CPU_ARCH" == "arm64" ]; then
      np_platform="--platform=linux/amd64"
    fi
    if [ "$container_pulled" = false ]; then
...
if [[ "$1" == "--start" ]]; then
  ...
  # Check if previous files exist (Tự dọn dẹp an toàn nếu không có container đang chạy)
  if [ -f "$process_id_file" ]; then
    OLD_PID=$(cat "$process_id_file" 2>/dev/null)
    if ! ps -p "$OLD_PID" > /dev/null 2>&1; then
      rm -f "$process_id_file" "$connection_state_file"
    fi
  fi
...
# Read the properties file and export variables safely (NO DANGEROUS EVAL)
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    # Bóc bỏ dấu nháy đơn/kép ngoài cùng nếu có
    if [[ "$value" =~ ^[\'\"](.*)[\'\"]$ ]]; then
