#!/usr/bin/env bash
#============================================================================
#  setup_vm.sh (2026 ULTIMATE MASTER - SYNCED 100% WITH VPS ENGINE)
#  Optimized for: Linux VM on Windows PC (VMware, VirtualBox, Hyper-V, KVM)
#  Hardening: Time-Drift Guard (Chrony), LVM Auto-Extend, Anti-NAT Home Router,
#             Docker Non-Root Permission, FlapGuard, Repocket Doctor, Dynamic KSM
#============================================================================
set -Eeuo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --auto-off)     AUTO_OFF_TIME="${2:-}"; shift 2 ;;
    --auto-off=*)   AUTO_OFF_TIME="${1#*=}"; shift ;;
...
GUEST_PKGS=("curl" "wget" "git" "unzip" "jq" "bc" "ca-certificates" "uuid-runtime" "cron" "logrotate" "net-tools" "inotify-tools" \
            "iptables-persistent" "netfilter-persistent" "chrony" "vnstat" "nload" "dnsutils" "util-linux" "e2fsprogs")

case "$VIRT" in
  vmware)
    GUEST_PKGS+=("open-vm-tools")
    log "Phat hien VMware Host -> Tich hop open-vm-tools" ;;
  oracle|kvm|qemu)
    GUEST_PKGS+=("qemu-guest-agent")
    log "Phat hien VirtualBox/KVM Host -> Tich hop qemu-guest-agent" ;;
  microsoft)
    GUEST_PKGS+=("hyperv-daemons")
    log "Phat hien Hyper-V Host -> Tich hop hyperv-daemons" ;;
esac
...
# Tự động mở rộng 100% dung lượng LVM trên Ubuntu Server
log "Kiem tra va tu dong mo rong phan vung o dia LVM..."
...
# Cấu hình Chrony & Time-Drift Guard cho Windows Host Sleep/Wake
cat << 'EOF_CHRONY' > /etc/chrony/chrony.conf
...
cat > /usr/local/bin/ii-timedrift-guard.sh <<'EOF_TIMEDRIFT'
#!/usr/bin/env bash
while true; do
...
if has_systemd; then
  cat > /etc/systemd/system/ii-timedrift-guard.service <<'EOF_TD_SVC'
[Unit]
...
EOF_TD_SVC
  ...
  systemctl enable --now ii-timedrift-guard.service 2>/dev/null || true
fi

log "Cau hinh DNS Direct-Upstream & Khoa bat bien chattr +i (Chong vSwitch ghi de)..."
UPSTREAM_DNS=""
...
# Safe shutdown script nếu có tham số --auto-off HH:MM
if [[ -n "$AUTO_OFF_TIME" ]]; then
  HOUR=$(echo "$AUTO_OFF_TIME" | cut -d: -f1)
  MIN=$(echo "$AUTO_OFF_TIME" | cut -d: -f2)
  cat > /usr/local/bin/ii-safe-shutdown.sh <<'EOF_SHUTDOWN'
#!/usr/bin/env bash
...
EOF_CRON
  ...
  # Bổ sung crontab tắt máy an toàn nếu có --auto-off
  if [[ -n "$AUTO_OFF_TIME" ]]; then
...
ROOTS=("$@")
if (( ${#ROOTS[@]} == 0 )); then ROOTS=(/opt /root /home /srv /home/ubuntu /home/opc /home/antoine); fi
...
# Tự động quét và kích hoạt toàn bộ các cụm node (Zero-Touch)
log "Tu dong phat hien va kich hoat toan bo cac cum node trong he thong..."
...
echo "============================= SETUP XONG (2026 UNIVERSAL VM MASTER) =============================="
